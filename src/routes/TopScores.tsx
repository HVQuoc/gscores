...
const TopScores = () => {
  const { data, error, isLoading } = useSWR("/api/reports/score-level/group", fetcher);
  ...
  return (
    <div className="w-full mx-auto mt-8">
      <Card>
        ...
        {error && <div className="text-red-600">Failed to load top scores.</div>}
        {data && (
              ...
              <tbody>
                {data.map((item: any, idx: number) => (
                  <tr key={item.sbd} className="border-t">
                    ...
                    <td className="px-3 py-2 font-semibold">{item.total_score}</td>
                    <td className="px-3 py-2">
                      <ul>
                        {Object.entries(item.subject_scores).map(([subject, score]) => (
                          <li key={subject}>
                            <span className="font-medium">{subject}:</span> {String(score)}
                          </li>
                        ))}
                      </ul>
                    </td>
